...
import { IntersectionDetector } from '../geometric/intersection-detector';
...
export class QuadTree {
    ...
    getRangePoints(range: Circle, rangePoints: Point[] = []): Point[] {
        if (!IntersectionDetector.RectToCircle(this.boundary, range)) {
            return rangePoints;
        }

        this.points.forEach(pt => {
            if (IntersectionDetector.PointToCircle(pt, range)) {
                rangePoints.push(pt);
            }
        });
